...
#Create or load a favorite dictionary
#Get a project_path
try:
    project_path = bpy.path.abspath('')
    #Create a setcolor_data folder
    data_path = os.path.join(project_path, 'setcolordata')
    if not os.path.exists(data_path):
        try:
            os.makedirs(data_path)
            print('Folder "setcolordata" created successfully')
        except OSError:
            print('SETCOLOR Addon: The folder for dictionary has not been created. Save the project and try again')
        ...
        if os.path.isfile(file_path):
            ...
            fav_dict={}
            print(f'The {file_path} is missing.')
        ...
        if os.path.isfile(id_path):
            ...
            fav_dict_id={}
            print(f'The {id_path} is missing.')
except OSError:
    print('SETCOLOR Addon: The folder for dictionary has not been created. Save the project and try again')
...
class SHADER_OT_RALCLASSIC(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if bpy.data.scenes["Scene"].new_ralmaterial:
...
class SHADER_OT_RALDESIGN(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if bpy.data.scenes["Scene"].new_designmaterial:
...
class SHADER_OT_RALEFFECT(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if bpy.data.scenes["Scene"].new_effectmaterial:
...
class SHADER_OT_PANTONE(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if bpy.data.scenes["Scene"].new_pantonematerial:
...
class SHADER_OT_NNPANTONE(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if bpy.data.scenes["Scene"].new_nnpantonematerial:
...
class SHADER_OT_NCS(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if bpy.data.scenes["Scene"].new_ncsmaterial:
...
class SHADER_OT_FAV(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if bpy.data.scenes["Scene"].new_favmaterial:
...
class SHADER_OT_LOAD(bpy.types.Operator):
    """Load dictionary if it was not possible to initially create a folder with the dictionary"""
    ...
    def execute(self, context):
        ...
        if not os.path.exists(data_path):
            try:
                os.makedirs(data_path)
                self.report({'INFO'}, 'Folder "setcolordata" created successfully')
            except OSError:
                self.report({'ERROR'}, 'The folder has not been created. Save the project and try again')
            ...
            if os.path.isfile(file_path):
                with open(file_path, 'rb') as file:
                    fav_dict = pickle.load(file)
                    self.report({'INFO'}, 'Loaded successfully')
            else:
                fav_dict={}
                self.report({'INFO'}, 'Dictionary is missing')
